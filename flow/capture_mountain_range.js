...
async function run() {
    ...
    try {
        ...
        console.log('Navigating to mountain range visualizer page...');
        await page.goto('https://localhost:8085/mountain_range.html', {
        ...
        if (dryIsoSvgElement) {
            const outputPathDry3D = path.join(__dirname, 'mountain_range_3d_dry.png');
        ...
        if (gridSvgElement) {
            const outputPath2D = path.join(__dirname, 'mountain_range_2d.png');
        ...
        if (isoSvgElement) {
            const outputPath3D = path.join(__dirname, 'mountain_range_3d.png');
        ...
        if (csSvgElement) {
            const outputPathCS = path.join(__dirname, 'mountain_range_cs.png');
