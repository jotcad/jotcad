...
int main() {
    ...
    // Run 60 steps at dt = 0.05s to simulate the same 3.0s total physical time as the baseline (15 steps * 0.2s)
    int total_sim_steps = 60;
    int export_stride = 4;      // Export every 4th step to get exactly 15 output frames
    
    float dt = 0.05f;           // 4x smaller time step for higher CFL stability
    float g = 2.5f;             
    float drag = 0.25f;         
    float K_infil = 0.5f;       
    float K_darcy = 0.6f;       
        ...
        for (int y = 0; y < GRID_SIZE; ++y) {
            for (int x = 0; x < GRID_SIZE; ++x) {
                if (h_surface[y][x] > 0.01f) {
                    ...
                    float fill_depth = h_surface[y][x];
    ...
    // REGRESSION ASSERTS (Step 60/Index 15)
    assert(history[0].H_soil[5][5] > 6.5f && history[0].H_soil[5][5] < 7.5f);
    assert(history[5].h_subsurface[5][4] > history[0].h_subsurface[5][4]);
    assert(history[15].H_soil[5][6] < history[0].H_soil[5][6]); // sapping has occurred!

    // Save as test_piping_dt.js
    std::ofstream out("test_piping_dt.js");
    out << "export const pipingDtData = {\n";
    out << "  grid_size: 10,\n";
    out << "  steps: [\n";
    ...
    std::cout << "SUCCESS: Piping time-step test completed successfully." << std::endl;
    return 0;
}
