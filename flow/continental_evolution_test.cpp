...
int main() {
    // Start Phase 1 on 256x256 grid (Doubled grid dimensions)
    const int GRID_LOW = 256;
    const int GRID_HIGH = 512;
    Orchestrator orch(GRID_LOW);
    Grid& g = orch.get_grid();

    // 1. Initialize the continent profile (256x256)
    std::cout << "Initializing irregular continental basement topography (256x256)..." << std::endl;
    ...
    // Configure Phase 1 with target grid size GRID_LOW (256)
    Phase* p1 = orch.add_phase("Bolide Impact", dt_tectonic, steps_tectonic, GRID_LOW);
    p1->add<ImpactCrater>(128.0f, 128.0f, 35.0f, 4.0f, 2.0f); // Crater center at 128,128 and radius 35 (unscaled absolute size)
    ...
    // 4. Setup Phase 2: Hydrological Erosion & Incision (Runs on upscaled 256x256 grid)
    const int steps_hydro = 120;
    float dt_hydro = 0.05f;
    // Configure Phase 2 with target grid size GRID_HIGH (256)
    ...
    // 4. Setup Phase 3: Coupled Eco-Vegetation (Runs on upscaled 512x512 grid)
    const int steps_eco = 2000;
    float dt_eco = 0.05f;
    // Configure Phase 3 with target grid size GRID_HIGH (512)
    ...
    p3->add<Landslide>(0.16f, 511, 511); // Lower repose angle, boundary sink at 511,511
    ...
    // Run Phase 1: Bolide Impact (128x128)
    std::cout << "Running Phase 1: Bolide Impact Excavation (128x128)..." << std::endl;
    ...
    // Inundation Transition: Flood ocean basin to sea level z = 0.0 (256x256)
    ...
    // Run Phase 2: Hydrological Incision (256x256)
    std::cout << "Running Phase 2: Hydrology & River Incision (256x256)..." << std::endl;
    ...
    // Run Phase 3: Coupled Eco-Vegetation (256x256)
    std::cout << "Running Phase 3: Eco-Vegetation Growth (256x256)..." << std::endl;
    ...
    // Export dataset
    std::cout << "Exporting continental evolution dataset to test_continent.js..." << std::endl;
    export_continent_data("test_continent.js", history, 128);
