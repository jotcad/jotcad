...
void apply_multipoint_island(HexGrid& g, float center_height, float ocean_depth, float noise_scale, float noise_amp, int num_seeds, float spread_radius) {
    ...
    // stays at least 12km away from the borders.
...
int main(int argc, char* argv[]) {
    ...
    std::vector<ClimateProfile> profiles = { ClimateRegistry::get_profile("Littoral Coastline") };
    ...
    for (const auto& profile : profiles) {
        ...
        for (int r = 0; r < SIZE_R; ++r) {
            for (int q = 0; q < SIZE_Q; ++q) {
                ...
                bool is_lake = (h_lake[r][q] > profile.lake_threshold);
                bool is_wetland = false;
                if (!is_lake && profile.check_wetland_groundwater && g.has_field<HexGroundwater>()) {
        ...
        if (generate_mode) {
            std::string ref_bin_save = get_write_path("hex_regression_large_ref_" + safe + ".bin");
            std::string ref_png_save = get_write_path("hex_regression_large_ref_" + safe + ".png");
        ...
        std::string ref_bin_path = "hex_regression_large_ref_" + safe + ".bin";
        {
            std::ifstream test_f(ref_bin_path);
            if (!test_f.is_open()) {
                ref_bin_path = "flow/hex_regression_large_ref_" + safe + ".bin";
            } else {
                test_f.close();
            }
        }
        ...
        if (!in_ref.is_open()) {
            ...
            std::cerr << "Run: './hex_regression_large_test --generate' first to record the baseline." << std::endl;
            return 1;
        }
        ...
        std::string current_png_save = get_write_path("hex_frames/frame_1000_" + safe + "_large.png");
        std::string diff_png_save = get_write_path("hex_regression_large_diff_" + safe + ".png");
    ...
    std::string stats_json_save = get_write_path("hex_regression_large_stats.json");
