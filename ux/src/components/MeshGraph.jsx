...
export function MeshGraph() {
  ...
  const [pos, setPos] = createSignal({ x: window.innerWidth - 340, y: window.innerHeight - 450 });
  ...
  onMount(() => {
    interact(nodeRef)
      ...
      .resizable({
        ...
        listeners: {
          move(event) {
            setSize(s => ({ width: event.rect.width, height: event.rect.height }));
  ...
  const getStatusColor = (type) => {
    switch (type) {
      case 'READ_START': return 'text-blue-400';
      case 'HANDLER_START': return 'text-yellow-400';
      case 'WORKING': return 'text-orange-400 animate-pulse';
  ...
  return (
    ...
    >
      <div class="p-3 rounded-t-xl bg-black/60 backdrop-blur-xl border-x-2 border-t-2 border-white/10 flex justify-between items-center cursor-move">
        <span class="text-[10px] font-black uppercase tracking-widest text-white/40">Mesh Pulse</span>
        <div class={`w-2 h-2 rounded-full ${blackboard.isConnected() ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500'}`} />
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-2 bg-black/40 backdrop-blur-md border-x-2 border-b-2 border-white/10 rounded-b-xl flex flex-col gap-1 custom-scrollbar">
        <Show when={pulses().length === 0}>
          <div class="p-4 text-center text-white/20 text-[10px] uppercase italic text-balance">Silent Mesh...</div>
        </Show>
          ...
          {(pulse) => (
            <div class="group flex flex-col p-2 rounded-lg bg-white/5 border border-white/5 hover:border-white/10 transition-colors shrink-0">
              <div class="flex justify-between items-start gap-2">
                <span class={`text-[10px] font-bold ${getStatusColor(pulse.payload.type)}`}>
                  {pulse.payload.type}
                </span>
