...
test('Partitioned Output Ports', async (t) => {
  ...
  await t.test('Writing to partitioned ports', async () => {
    ...
    await vfs.writeData(baseSelector.withOutput('file'), new TextEncoder().encode('raw bytes data'));
    await vfs.writeData(baseSelector.withOutput('status'), { status: 'success', size: 14 });
  ...
  await t.test('Reading from specific ports', async () => {
    ...
    const fileData = await vfs.readData(baseSelector.withOutput('file'));
    ...
    const statusData = await vfs.readData(baseSelector.withOutput('status'));
  ...
  await t.test('Reading missing ports', async () => {
    const missingData = await vfs.readData(baseSelector.withOutput('nonexistent'));
