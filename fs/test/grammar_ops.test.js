...
test('Geometric Grammar Integration', async (t) => {
  ...
  await t.test(
    ...
    async () => {
      ...
      const firstTwo = {
        path: 'op/nth',
        parameters: { source: points, indices: [0, 1] },
      };
      const origin = { path: 'shape/origin', parameters: {} };
      const group = {
        path: 'op/group',
        parameters: { sources: [origin, firstTwo] },
      };
      ...
      console.log(
        `[Test Grammar] Result has ${vCount} vertices and ${lCount} segments.`
      );
      ...
      assert.ok(
        lines[0].includes('v 0.000000 0.000000 0.000000'),
