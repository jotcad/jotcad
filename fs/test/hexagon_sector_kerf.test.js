...
test('Complex Mesh Expression: Hexagon Sector with Kerf', async (t) => {
    ...
    t.after(async () => {
        ...
        await fs.rm(path.resolve('.vfs_storage_sector-ops'), { recursive: true, force: true }).catch(() => {});
    });
    ...
    clientVfs = new VFS({ id: 'sector-client', storage: new DiskStorage(STORAGE_JS) });
    const mesh = new MeshLink(clientVfs, [`http://localhost:${PORT_OPS}`], { localUrl: `http://localhost:${PORT_CLIENT}` });
    ...
    await t.test('should resolve nested mesh expression for kerfed sector', async () => {
                                            ...
                                            { 
                                                ...
                                                parameters: { 
                                                    ...
                                                    source: {
                                                        path: 'op/points',
                                                        parameters: {
                                                            source: { path: 'shape/hexagon', parameters: { radius: 100 } }
                                                        }
                                                    }
                                                } 
                                            }
        ...
        const pdfData = await clientVfs.readData(expression.path, expression.parameters);
        
        assert.ok(pdfData, 'Should return PDF data');
        console.log(`[Test Sector] Received data type: ${pdfData.constructor.name}, length: ${pdfData.length}`);
