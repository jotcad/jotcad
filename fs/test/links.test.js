...
test('VFS Link Resolution & Metadata Aggregation', async (t) => {
  ...
  await t.test('should transparently follow multi-hop links', async () => {
    ...
    assert.strictEqual(
      text,
      'TERMINAL',
      'Should return data from the end of the chain'
    );
  });
  ...
  await t.test('should handle circular links gracefully', async () => {
    vfs.registerProvider('loop/A', async () =>
      JSON.stringify({ geometry: 'vfs:/loop/B' })
    );
    vfs.registerProvider('loop/B', async () =>
      JSON.stringify({ geometry: 'vfs:/loop/A' })
    );
    ...
    try {
      ...
      assert.ok(
        err.message.includes('recursion depth exceeded'),
  ...
  await t.test('should return non-link JSON as terminal data', async () => {
    vfs.registerProvider('data/json', async () =>
      JSON.stringify({ just: 'data' })
    );
