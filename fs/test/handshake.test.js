...
test(
  ...
  async (t) => {
    ...
    await t.test('should negotiate DIRECT linkage on localhost', async () => {
      ...
      assert.strictEqual(
        nodeB.mesh.reverseRegistry.listeners.has('a'),
        false,
        'Should NOT have a reverse listener for direct peer'
      );
    });
    ...
    await t.test(
      ...
      async () => {
        ...
        assert.ok(
          nodeB.mesh.reverseRegistry.listeners.has('a'),
          'Node B SHOULD have a reverse listener for unreachable peer'
        );
