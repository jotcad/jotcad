...
test('Multi-Environment VFS (REST + SSE)', { timeout: 20000 }, async (t) => {
    ...
    const server = http.createServer((req, res) => {
        ...
        if (req.url === '/' || req.url === '/index.html') {
            ...
            filePath = path.join(__dirname, '..', 'src', req.url.replace('/src/', ''));
        } else {
    ...
    await t.test('cascading coordination via REST + SSE', { timeout: 10000 }, async () => {
        ...
        await vfs.write('final-result', {}, Readable.from([Buffer.from('Success!')]));
