...
test('AgentNode abstraction', async (t) => {
  ...
  const processNode = new Node(vfs, {
    ...
    async execute({ out }) {
      ...
      await out.write(Readable.from(['Completed']), { status: 'success', timestamp: 123 });
    }
  });
  ...
  await t.test('Node resolves inputs from other blackboard locations', async () => {
    ...
    await vfs.write('config/box', { id: 'b1' }, Readable.from([JSON.stringify({ size: 50 })]));
  ...
  await t.test('Node can reparameterize output writes', async () => {
    ...
    const stream = await vfs.read('process/result', { status: 'success', timestamp: 123 });
