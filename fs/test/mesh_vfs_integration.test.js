...
test('Decentralized Mesh-VFS Integration', async (t) => {
    ...
    t.after(async () => {
        ...
        if (nodeC) { nodeC.stopServer(); nodeC.server.close(); await nodeC.vfs.close(); }
        
        await fs.rm(TEST_STORAGE_A, { recursive: true, force: true }).catch(() => {});
        await fs.rm(TEST_STORAGE_B, { recursive: true, force: true }).catch(() => {});
        await fs.rm(TEST_STORAGE_C, { recursive: true, force: true }).catch(() => {});
