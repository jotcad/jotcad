...
test('Mesh Pub-Sub: Propagation & Backflow Prevention', async (t) => {
    ...
    const connect = (mA, mB) => {
        const peerA = {
            ...
            notify: (s, p, st) => { mB.notify(s, p, st); return Promise.resolve(); }
        };
        const peerB = {
            id: mA.vfs.id,
            subscribe: (s, e, st) => { mA.addInterest(mB.vfs.id, s, e, st); return Promise.resolve(); },
            notify: (s, p, st) => { mA.notify(s, p, st); return Promise.resolve(); }
        };
    ...
    await t.test('subscription should propagate through the chain', async () => {
        ...
        assert.ok(bEntry && bEntry.subs.has('node-A'), 'Node B should record Node A interest');

        // Verify C knows B is interested (propagation)
        const cEntry = meshC.interests.get(topicString);
        assert.ok(cEntry && cEntry.subs.has('node-B'), 'Node C should record Node B interest');
    });
