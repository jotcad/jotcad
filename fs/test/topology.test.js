...
test('Mesh Topology Discovery', async (t) => {
    ...
    t.after(() => {
        ...
        vfsA.close(); vfsB.close(); vfsC.close();
    });
    ...
    const connect = (mA, mB) => {
        ...
        const peerA = {
            id: mB.vfs.id, reachability: 'DIRECT',
            subscribe: (s, e, st) => { mB.addInterest(mA.vfs.id, s, e, st); return Promise.resolve(); },
            notify: (s, p, st) => { 
                return Promise.resolve(mA.notify(s, p, st)); 
            } 
        };
        const peerB = {
            id: mA.vfs.id, reachability: 'DIRECT',
            subscribe: (s, e, st) => { mA.addInterest(mB.vfs.id, s, e, st); return Promise.resolve(); },
            notify: (s, p, st) => { 
    ...
    await t.test('Node A should receive topology from Node C via Node B', async () => {
        ...
        meshA.notify = (selector, payload, stack) => {
            console.log(`[Test] Node A received pulse: ${selector.path} from ${payload.peer}`);
        ...
        const triggerHeartbeat = (m, name) => {
            const neighbors = [...m.peers.values()].map(p => ({ id: p.id, reachability: p.reachability }));
            const sel = { path: 'sys/topo', parameters: { id: m.vfs.id } };
            console.log(`[Test] Triggering heartbeat from ${name} (${m.vfs.id})...`);
        ...
        assert.ok(receivedTopo.has('node-C'), 'Node A should have received Node C topology');
        assert.ok(receivedTopo.has('node-B'), 'Node A should have received Node B topology');
    });
});
