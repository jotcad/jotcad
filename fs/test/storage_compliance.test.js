...
async function runComplianceSuite(t, storage, cleanup) {
    ...
    await t.test('should enforce size verification (Aborted Stream)', async () => {
        ...
        try {
            ...
            assert.ok(err.message.includes('Expected 100') || err.message.includes('aborted'));
        }

        // Ensure it wasn't cached
        const exists = await storage.has(badCid);
        assert.strictEqual(exists, false, 'Corrupted data should not be accessible');
    });
