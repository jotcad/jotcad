...
test('VFS Aborted Stream Detection', async (t) => {
  await t.test(
    'should reject stream that ends before expected length',
    async () => {
      const vfs = new VFS({
        id: 'integrity-vfs-1',
      ...
      assert.strictEqual(
        await vfs.storage.has(cid),
        false,
        'Partial data should not be cached'
      );
    }
  );
  ...
  await t.test('should allow stream that matches expected length', async () => {
    ...
    assert.ok(
      capturedCID !== null,
      'Should have captured a CID via state event'
    );
