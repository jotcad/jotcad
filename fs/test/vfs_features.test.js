...
test('VFS Core Features', async (t) => {
  ...
  await t.test(
    'writeData() and readData() handle JSON automatically',
    async () => {
      const vfs = new VFS({
        id: 'test-json',
        storage: new DiskStorage(STORAGE),
      });
      await vfs.init();
  ...
  await t.test(
    'read() deduplication prevents multiple redundant mesh calls',
    async () => {
      ...
      assert.strictEqual(
        meshCallCount,
        1,
        'Should only call mesh once for same selector'
      );
