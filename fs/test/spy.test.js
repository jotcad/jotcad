...
test('Discovery Protocol (Spy)', async (t) => {
    ...
    await t.test('setup network', async () => {
        ...
        meshA = new MeshLink(vfsA, ['http://localhost:19902'], { localUrl: 'http://localhost:19901' });
        meshB = new MeshLink(vfsB, ['http://localhost:19901'], { localUrl: 'http://localhost:19902' });
    ...
    await t.test('spy returns multiplexed VFS bundle', async () => {
        ...
        const combined = Buffer.concat(chunks.map(c => Buffer.from(c))).toString();
        
        console.log('COMBINED:', combined);

        // Ensure both Node A and Node B results are present
        assert.ok(combined.includes('"shape/box"'), 'Should contain box selector');
        assert.ok(combined.includes('"shape/sphere"'), 'Should contain sphere selector');
