...
test('parseVFSBundle', async () => {
    const chunk1 = VFS.formatVFSChunk({ path: 'a' }, new TextEncoder().encode('hello'));
    const chunk2 = VFS.formatVFSChunk({ path: 'b' }, new TextEncoder().encode('world'));
    ...
    for await (const item of VFS.parseVFSBundle(stream)) {
        results.push({ selector: item.selector, text: new TextDecoder().decode(item.data) });
    }
