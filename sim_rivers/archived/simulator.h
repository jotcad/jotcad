...
struct Cell {
    ...
    float soil;             // Mineral soil thickness (m) (total: soil_clay + soil_gravel)
    float soil_clay;        // Clay/silt thickness (m)
    float soil_gravel;      // Sand/gravel thickness (m)
    float water;            // Surface water depth (m)
    float groundwater;      // Subsoil groundwater storage (m)
    float sediment;         // Suspended sediment (m) (total: sediment_clay + sediment_gravel)
    float sediment_clay;    // Suspended clay (m)
    float sediment_gravel;  // Suspended gravel (m)
