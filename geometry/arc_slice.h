...
static GeometryId ArcSlice2(Assets& assets, Shape& shape, EK::FT start, EK::FT end) {
  CGAL::Plane_3<EK> plane(0, 0, 1, 0);
  CGAL::Ray_3<EK> ray_3(CGAL::Point_3<EK>(0, 0, 0), CGAL::Direction_3<EK>(1, 0, 0));
  ...
  while (!done) {
    for (const auto& [s, t] : geometry.segments_) {
      ...
      if (last) {
        ...
        if (result) {
          if (const CGAL::Point_3<EK>* p = std::get_if<CGAL::Point_3<EK>>(&*result)) {
            sliced.AddSegment(*last, *p);
            last.reset();
          } else if (const CGAL::Segment_3<EK>* s = std::get_if<CGAL::Segment_3<EK>>(&*result)){
          ...
          sliced.segments_.emplace_back(sliced.AddVertex(s3.source()), sliced.AddVertex(s3.target()));
          last = s3.target();
        }
      } else {
