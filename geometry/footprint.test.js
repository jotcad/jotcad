...
describe('footprint', () => {
  it('should create a 2D footprint from a 3D box', async () => {
    await withTestAssets('footprint of a 3d box', async (assets) => {
      ...
      const image = await renderPng(assets, footprintShape, {
        width: 100,
        height: 100,
      });
      assert.ok(
        await testPng(`${import.meta.dirname}/footprint.test.box.png`, image)
      );
    });
  });
  ...
  it('should create a 2D footprint from an object with a hole', async () => {
    await withTestAssets(
      ...
      async (assets) => {
        ...
        const hollowSquareShape = cut3(assets, largeBox, [smallBox]);
        ...
        const image = await renderPng(assets, footprintShapeWithHole, {
          width: 100,
          height: 100,
        });
        assert.ok(
          await testPng(
            `${import.meta.dirname}/footprint.test.hollow_square.png`,
            image
          )
        );
      }
