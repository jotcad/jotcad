...
describe('footprint', () => {
  it('should create a 2D footprint from a 3D box', withTestAssets(async (assets) => {
    const box = Box3(10, 10, 10); // A 10x10x10 cube
    const footprintShape = footprint(box);

    // Verify that a valid geometry was returned
    assert.ok(footprintShape.geometryId !== undefined && footprintShape.geometryId !== null, 'Footprint should return a valid geometry ID');
    ...
    assert.equal(footprintGeometry.vertices_.length, 4, 'Footprint should have 4 vertices');
    assert.equal(footprintGeometry.faces_.length, 1, 'Footprint should have 1 face');
    assert.equal(footprintGeometry.faces_[0].first.length, 4, 'The face should have 4 vertices');
    assert.equal(footprintGeometry.faces_[0].second.length, 0, 'The face should have no holes');
  ...
  it('should create a 2D footprint from an object with a hole', withTestAssets(async (assets) => {
    ...
    const hollowSquareShape = { geometryId: customGeometryId, tfId: assets.Id(assets.identity()) };

    const footprintShapeWithHole = footprint(hollowSquareShape);

    assert.ok(footprintShapeWithHole.geometryId !== undefined && footprintShapeWithHole.geometryId !== null, 'Footprint with hole should return a valid geometry ID');

    const footprintGeometryWithHole = assets.Get(footprintShapeWithHole.geometryId);

    // Expect 8 vertices (4 outer, 4 inner)
    assert.equal(footprintGeometryWithHole.vertices_.length, 8, 'Footprint with hole should have 8 vertices');
    // Expect 1 face with 1 hole
    assert.equal(footprintGeometryWithHole.faces_.length, 1, 'Footprint with hole should have 1 face');
    assert.equal(footprintGeometryWithHole.faces_[0].first.length, 4, 'Outer boundary of face should have 4 vertices');
    assert.equal(footprintGeometryWithHole.faces_[0].second.length, 1, 'Face should have 1 hole');
    assert.equal(footprintGeometryWithHole.faces_[0].second[0].length, 4, 'The hole should have 4 vertices');

    const outputPath = assets.getPath('footprint_hollow_square.png');
    await renderPng(footprintShapeWithHole, outputPath, { width: 100, height: 100 });
