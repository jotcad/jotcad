...
static GeometryId ClipOpen(Assets& assets, Shape& shape,
  ...
  for (Shape& tool_shape : tool_shapes) {
    tool_shape.Walk([&](Shape& tool_sub_shape) {
      if (tool_sub_shape.GetBooleanTag("isPlane")) {
        ...
        std::cout << "ClipOpen: Plane detected: " << plane << std::endl;
      ...
      std::cout << "ClipOpen: Mesh tool detected." << std::endl;
      ...
      // We use clip to keep only what's inside. clip_volume(false) ensures we
      // don't close the result.
      CGAL::Polygon_mesh_processing::clip(
