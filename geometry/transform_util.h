...
static EK::FT compute_scaling_factor(double value) {
  return CGAL::simplest_rational_in_interval<EK::FT>(value * 0.999,
                                                     value * 1.001);
}

static EK::FT compute_translation_offset(double value) {
  return CGAL::simplest_rational_in_interval<EK::FT>(value - 0.001,
                                                     value + 0.001);
}
...
disorient_plane_along_z(Plane source) {
  ...
  CGAL::Point_3<EK> s = source.to_3d(CGAL::Point_2<EK>(0, 0));
  transform = transform * translate(CGAL::Point_3<EK>(0, 0, 0) - s);
  return transform;
}
...
static CGAL::Aff_transformation_3<typename CGAL::Kernel_traits<Point_3>::Kernel>
computeInverseSegmentTransform(
    const Point_3& start, const Point_3& end,
    const typename CGAL::Kernel_traits<Point_3>::Kernel::Vector_3& normal) {
  Point_3 zero(0, 0, 0);
  CGAL::Aff_transformation_3<typename CGAL::Kernel_traits<Point_3>::Kernel>
      align(CGAL::IDENTITY);
  disorient_along_z(end - start, normal, align);
  return CGAL::Aff_transformation_3<
      typename CGAL::Kernel_traits<Point_3>::Kernel>(align *
...
static void transformPolygonWithHoles(
  ...
  polygon = CGAL::Polygon_with_holes_2<EK>(
      output_boundary, output_holes.begin(), output_holes.end());
}
...
static void transformPolygonsWithHoles(
    ...
    const EK::Plane_3& input_plane, const EK::Plane_3& output_plane,
    const CGAL::Aff_transformation_3<EK>& transform) {
  for (CGAL::Polygon_with_holes_2<EK>& polygon : polygons) {
