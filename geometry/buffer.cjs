...
var S = [],
  R = [],
  Sr = typeof Uint8Array < 'u' ? Uint8Array : Array,
...
function Mr(p) {
  ...
  for (B = 0; B < o; B += 4)
    (f =
      (R[p.charCodeAt(B)] << 18) |
      (R[p.charCodeAt(B + 1)] << 12) |
      (R[p.charCodeAt(B + 2)] << 6) |
      R[p.charCodeAt(B + 3)]),
      (a[c++] = (f >> 16) & 255),
      (a[c++] = (f >> 8) & 255),
      (a[c++] = f & 255);
  return (
    y === 2 &&
      ((f = (R[p.charCodeAt(B)] << 2) | (R[p.charCodeAt(B + 1)] >> 4)),
      (a[c++] = f & 255)),
    y === 1 &&
      ((f =
        (R[p.charCodeAt(B)] << 10) |
        (R[p.charCodeAt(B + 1)] << 4) |
        (R[p.charCodeAt(B + 2)] >> 2)),
      (a[c++] = (f >> 8) & 255),
      (a[c++] = f & 255)),
    a
  );
...
function Dr(p, f, l) {
  for (var x, y = [], a = f; a < l; a += 3)
    (x =
      ((p[a] << 16) & 16711680) + ((p[a + 1] << 8) & 65280) + (p[a + 2] & 255)),
      y.push(kr(x));
  return y.join('');
}
function br(p) {
  for (
    var f, l = p.length, x = l % 3, y = [], a = 16383, c = 0, o = l - x;
    c < o;
    c += a
  )
  ...
  return (
    x === 1
      ? ((f = p[l - 1]), y.push(S[f >> 2] + S[(f << 4) & 63] + '=='))
      : x === 2 &&
        ((f = (p[l - 2] << 8) + p[l - 1]),
        y.push(S[f >> 10] + S[(f >> 4) & 63] + S[(f << 2) & 63] + '=')),
    y.join('')
  );
}
var V = {};
/*! ieee754. BSD-3-Clause License. Feross Aboukhadijeh <https://feross.org/opensource> */ V.read =
  function (p, f, l, x, y) {
    var a,
      ...
      A = p[f + F];
    for (
      F += _, a = A & ((1 << -I) - 1), A >>= -I, I += o;
      I > 0;
      a = a * 256 + p[f + F], F += _, I -= 8
    );
    for (
      c = a & ((1 << -I) - 1), a >>= -I, I += x;
      I > 0;
      c = c * 256 + p[f + F], F += _, I -= 8
    );
    if (a === 0) a = 1 - T;
    else {
...
V.write = function (p, f, l, x, y, a) {
  var c,
    ...
    I = (1 << T) - 1,
    F = I >> 1,
    _ = y === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0,
    A = x ? 0 : a - 1,
    b = x ? 1 : -1,
    $ = f < 0 || (f === 0 && 1 / f < 0) ? 1 : 0;
  for (
    f = Math.abs(f),
      isNaN(f) || f === 1 / 0
        ...
        : ((c = Math.floor(Math.log(f) / Math.LN2)),
          ...
          c + F >= 1 ? (f += _ / B) : (f += _ * Math.pow(2, 1 - F)),
          f * B >= 2 && (c++, (B /= 2)),
          c + F >= I
            ? ((o = 0), (c = I))
            : c + F >= 1
    ...
    y >= 8;
    p[l + A] = o & 255, A += b, o /= 256, y -= 8
  );
  for (
    c = (c << y) | o, T += y;
    T > 0;
    p[l + A] = c & 255, A += b, c /= 256, T -= 8
  );
  p[l + A - b] |= $ * 128;
};
/*!
 ...
 */ (function (p) {
  ...
  o.poolSize = 8192;
  function B(i, r, t) {
    if (typeof i == 'string') return _(i, r);
    if (ArrayBuffer.isView(i)) return b(i);
    if (i == null)
      throw new TypeError(
    ...
    if (
      C(i, ArrayBuffer) ||
      (i && C(i.buffer, ArrayBuffer)) ||
      (typeof SharedArrayBuffer < 'u' &&
        (C(i, SharedArrayBuffer) || (i && C(i.buffer, SharedArrayBuffer))))
    )
      return $(i, r, t);
    if (typeof i == 'number')
      throw new TypeError(
  ...
  (o.isBuffer = function (r) {
    ...
    (o.compare = function (r, t) {
      if (
        (C(r, Uint8Array) && (r = o.from(r, r.offset, r.byteLength)),
  ...
  function X(i, r) {
    ...
    if (ArrayBuffer.isView(i) || C(i, ArrayBuffer)) return i.byteLength;
    if (typeof i != 'string')
      throw new TypeError(
  ...
  function Br(i) {
    ...
    let t = '',
      n = 0;
    for (; n < r; )
      t += String.fromCharCode.apply(String, i.slice(n, (n += Z)));
    return t;
  }
  function Er(i, r, t) {
    let n = '';
  ...
  (o.prototype.readUintLE = o.prototype.readUIntLE =
    ...
    (o.prototype.readUint16LE = o.prototype.readUInt16LE =
      function (r, t) {
        return (
          (r = r >>> 0), t || m(r, 2, this.length), this[r] | (this[r + 1] << 8)
        );
      }),
    (o.prototype.readUint16BE = o.prototype.readUInt16BE =
      function (r, t) {
        return (
          (r = r >>> 0), t || m(r, 2, this.length), (this[r] << 8) | this[r + 1]
        );
      }),
    ...
    (o.prototype.readFloatLE = function (r, t) {
      return (
        (r = r >>> 0), t || m(r, 4, this.length), l.read(this, r, !0, 23, 4)
      );
    }),
    (o.prototype.readFloatBE = function (r, t) {
      return (
        (r = r >>> 0), t || m(r, 4, this.length), l.read(this, r, !1, 23, 4)
      );
    }),
    (o.prototype.readDoubleLE = function (r, t) {
      return (
        (r = r >>> 0), t || m(r, 8, this.length), l.read(this, r, !0, 52, 8)
      );
    }),
    (o.prototype.readDoubleBE = function (r, t) {
      return (
        (r = r >>> 0), t || m(r, 8, this.length), l.read(this, r, !1, 52, 8)
      );
    });
  ...
  function Ir(i, r, t) {
    k(r, 'offset'),
      (i[r] === void 0 || i[r + t] === void 0) && P(r, i.length - (t + 1));
  }
      ...
      throw (
        (u > 3
          ? r === 0 || r === BigInt(0)
            ...
            : (s = `>= -(2${h} ** ${(u + 1) * 8 - 1}${h}) and < 2 ** ${
                (u + 1) * 8 - 1
              }${h}`)
          : (s = `>= ${r}${h} and <= ${t}${h}`),
        new M.ERR_OUT_OF_RANGE('value', s, i))
      );
    }
    Ir(n, e, u);
  }
  ...
  function P(i, r, t) {
    throw Math.floor(i) !== i
      ? (k(i, t), new M.ERR_OUT_OF_RANGE(t || 'offset', 'an integer', i))
      : r < 0
      ? new M.ERR_BUFFER_OUT_OF_BOUNDS()
      : new M.ERR_OUT_OF_RANGE(t || 'offset', `>= ${t ? 1 : 0} and <= ${r}`, i);
  }
