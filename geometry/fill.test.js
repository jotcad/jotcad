...
describe('fill', () => {
  it('should fill a triangle', async () => {
    await withTestAssets('should fill a triangle', async (assets) => {
      ...
      assert.ok(
        await testPng(`${import.meta.dirname}/fill.test.triangle.png`, image)
      );
    });
  });
  ...
  it('should fill the perimeter of a hollow ring', async () => {
    await withTestAssets(
      ...
      async (assets) => {
        ...
        assert.ok(
          await testPng(
            `${import.meta.dirname}/fill.test.ring_outline.png`,
            outlineImage
          )
        );
        ...
        assert.ok(
          await testPng(`${import.meta.dirname}/fill.test.ring.png`, image)
        );
      }
    );
  });
