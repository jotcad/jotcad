...
static json decode_jcb_impl(const std::vector<uint8_t>& buf, size_t& offset) {
    ...
    throw JCBParseException("JCB Decode: Invalid Tag");
}
...
std::string vfs_hash256(const std::vector<uint8_t>& data) {
    ...
    br_sha256_init(&ctx);
    br_sha256_update(&ctx, data.data(), data.size());
