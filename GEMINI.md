...
- **EXPLICIT UX VFS ACCESS**: The UX layer (via `VFSManager` and `blackboard`) MUST use explicit methods that target the correct VFS entry point: `readSelectorData`/`readSelector` for Recipes and `readCIDData`/`readCID` for Content. Generic `read`/`readData` wrappers are prohibited to prevent "string-guessing" regressions.
...
- **BUILD SYSTEM INTEGRITY**: Do NOT compile files using direct compiler invocations (e.g., `g++ ...`). Always compile and build via the system `Makefile`. Ensure that EVERY `Makefile` handles dependencies properly: it MUST both generate `.d` files using `-MMD -MP` AND include them using `-include $(wildcard $(OBJ_DIR)/*.d)` (or equivalent `$(DEPS)`). Sub-Makefiles (like `geo/test/Makefile`) MUST explicitly declare prerequisite rules delegating to parent Makefiles (e.g., `$(REG_OBJS): FORCE; $(MAKE) -C ..`) so that stale parent objects are automatically recompiled when headers change without requiring `make clean`.
