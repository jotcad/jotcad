...
struct SmoothOp : P {
    ...
    static void execute(fs::VFSNode* vfs, const fs::Selector& fulfilling, const Shape& in, 
        ...
        double ms_mesh = std::chrono::duration<double, std::milli>(t_mesh - t_start).count();
        ...
        double ms_roi = std::chrono::duration<double, std::milli>(t_roi - t_mesh).count();
        ...
        double ms_loop = std::chrono::duration<double, std::milli>(t_end - t_roi).count();
        std::cout << "[SmoothOp Profile] Total: " << total_ms << "ms (Mesh: " << ms_mesh << "ms, ROI: " << ms_roi << "ms, Loop+Mat: " << ms_loop << "ms, Vertices: " << mesh.number_of_vertices() << ", Iterations: " << iterations << ")" << std::endl;
