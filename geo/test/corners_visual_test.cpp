...
int main() {
    ...
    std::cout << "  - Testing 'eachCorner'..." << std::endl;
    Selector each_addr = Selector{"jot/eachCorner", {{"$in", box_addr}, {"proxy", true}}}.with_output("$out");
    Processor::execute(&vfs, each_addr);

    Shape each_out = vfs.read<Shape>(each_addr);
    if (each_out.components.size() != 4) {
        std::cerr << "    ❌ FAIL: eachCorner expected 4 components for a box, got " << each_out.components.size() << std::endl;
        return 1;
    }
    ...
    std::ofstream out2("actual/eachCorner_test.png", std::ios::binary);
    out2.write((char*)png2.data(), png2.size());
    out2.close();
    std::cout << "    ✅ Saved actual/eachCorner_test.png" << std::endl;
