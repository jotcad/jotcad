...
int main() {
    ...
    // 1. Sphere (width 10)
    Selector orb_sel = Selector{"jot/Orb", {{"width", 10.0}}}.with_output("$out");
    OrbOp<>::execute(&vfs, orb_sel, Interval{-5.0, 5.0}, Interval{0, 0}, Interval{0, 0}, 0.1);
    ...
    vfs.verify_well_formed_solid(o_geo, "Sphere (w=10)");

    // 2. Ellipsoid (20x10x5 override)
    Selector e_sel = Selector{"jot/Orb", {{"width", 20.0}, {"height", 10.0}, {"depth", 5.0}}}.with_output("$out");
    OrbOp<>::execute(&vfs, e_sel, Interval{-10.0, 10.0}, Interval{-5.0, 5.0}, Interval{-2.5, 2.5}, 0.1);
