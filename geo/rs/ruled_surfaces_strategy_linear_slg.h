...
void LinearSearchSlg<Objective>::generate(const PolygonalChain& p,
  ...
  // Fast path for K=1 (default for CAD ruling)
  if (options_.max_total_paths <= 1) {
    for (bool is_rev : {false, true}) {
      PolygonalChain current_q = q;
      if (is_rev) std::reverse(current_q.begin(), current_q.end());

      internal::AdjacencyList graph = internal::build_graph_slg(p, current_q, objective_);
      internal::NodeIndex start_node = 0;
      internal::NodeIndex end_node = internal::slg_node_to_index(p.size() - 1, current_q.size() - 1, current_q.size());
      
      internal::PathInfo path = internal::dijkstra(graph, start_node, end_node);
      if (path.is_valid()) {
        Mesh mesh = internal::soup_to_mesh(internal::reconstruct_triangulation_slg(p, current_q, path.steps));
        if (visitor.OnPermutation(mesh) == RuledSurfaceVisitor::kStop) return;

        if (options_.stitch) {
          auto stitched = internal::stitch_and_calculate_cost(mesh, p, current_q, objective_);
          if (stitched) {
            if (visitor.OnValidMesh(stitched->first, stitched->second) == RuledSurfaceVisitor::kStop) return;
          }
        } else {
          double cost = objective_.calculate_cost(mesh);
          if (visitor.OnValidMesh(mesh, cost) == RuledSurfaceVisitor::kStop) return;
        }
      }
    }
    SolutionStats final_stats;
    final_stats.paths_evaluated = 1;
    visitor.OnFinish(final_stats);
    return;
  }

  // Fallback to Yen for K > 1
  ...
  while ((!pq.empty() || !sidelined_generators.empty()) &&
    ...
    if (options_.stitch) {
      auto stitched_result =
          internal::stitch_and_calculate_cost(mesh, top->p_chain, top->q_chain, objective_);

      if (stitched_result) {
        if (visitor.OnValidMesh(stitched_result->first, stitched_result->second) ==
            RuledSurfaceVisitor::kStop)
          break;
      }
    } else {
      double cost = objective_.calculate_cost(mesh);
      if (visitor.OnValidMesh(mesh, cost) == RuledSurfaceVisitor::kStop) break;
    }
