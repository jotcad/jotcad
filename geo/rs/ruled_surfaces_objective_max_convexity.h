...
namespace internal {
inline double concavity_cost(const PointCgal& p1, const PointCgal& p2,
  ...
  if (n1.squared_length() < kEpsilon * kEpsilon ||
      n2.squared_length() < kEpsilon * kEpsilon) {
    return 0.0;
  }
  return (n1 * n2 < 0) ? 1.0 : 0.0;
}
}  // namespace internal

struct MaxConvexity : public Objective {
  double calculate_cost(const Mesh& mesh) const override {
    ...
    for (auto edge_id : mesh.edges()) {
      ...
      total_cost += internal::concavity_cost(p1, p2, p3, p4);
    }
    return total_cost;
  }
