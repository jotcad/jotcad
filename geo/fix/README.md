...
* **[assert_mesh.h](file:///home/brian/github/jotcad_ez/geo/fix/assert_mesh.h)**: Mesh Invariant Predicates & Assertions (`MeshStatus`, `check_corefinement_preconditions`, `check_solid_mesh`, `check_open_or_closed_mesh`, `is_well_formed_closed_mesh`, `is_well_formed_open_or_closed_mesh`, `assert_well_formed_closed_mesh`, `assert_well_formed_open_or_closed_mesh`): evaluates mesh integrity ordered strictly by increasing computational cost ($O(1) \to O(N) \to O(N \log N)$), returning typed `MeshStatus` enums and providing hard invariant assertions.
